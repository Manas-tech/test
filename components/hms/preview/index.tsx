...
export const PreviewScreen: React.FC<{ token: string }> = ({ token }) => {
  ...
  return (
    <div className="bg-white rounded-lg md:p-8 p-4 flex md:flex-row flex-col border border-gray-200 shadow-md">
      <PreviewContainer name={name} />
      <div className="w-[320px] flex flex-col md:ml-8 md:mt-0 mt-4 justify-between">
        <div>
          <p className="font-bold text-2xl my-0 text-gray-900">Welcome {name}</p>
          <p className="text-gray-500">Preview your video and audio before joining the stage</p>
        </div>
        <form
          onSubmit={e => {
            e.preventDefault();
        ...
        >
          <input
            ...
            required
            maxLength={20}
            className="w-full text-md bg-white text-gray-900 border border-gray-300 rounded-lg placeholder:text-gray-400 h-10 pl-2 focus:outline-none focus:bg-gray-100 focus:ring-2 focus:ring-brand-300"
            onChange={e => {
          ...
          <div className="flex space-x-4 mt-4">
            ...
            <Button
              disabled={!enableJoin}
              type="submit"
              className="bg-brand-300 text-white hover:bg-brand-400"
            >
...
const PreviewContainer: React.FC<{ name: string }> = ({ name }) => {
  ...
  return (
    <div className="w-[300px] h-[300px] relative flex justify-center items-center bg-gray-100 rounded-lg border border-gray-200">
      {localPeer ? (
        <>
