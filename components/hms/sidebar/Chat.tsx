...
const Chat = () => {
  ...
  return (
    <>
      <div
        id="chat-feed"
        className="h-full overflow-y-scroll p-4 bg-white border-b border-gray-200"
      >
        {msgs.length > 0 ? (
          msgs.map(m => (
            <div key={m.id} className="flex items-start w-full mb-5 relative">
              ...
              <div className="flex flex-col flex-grow">
                ...
                <div className="pl-2 bg-gray-100 text-gray-800 break-words w-[220px] mt-1 rounded-lg py-2 px-3 shadow-sm border border-gray-200">
                  {m.message}
                </div>
              </div>
          ...
          <div className="h-full flex justify-center items-center">
            <div className="w-full flex flex-col items-center text-center">
              <div className="md:w-[80px] md:h-[80px] w-[50px] h-[50px] bg-brand-100 rounded-full text-brand-300 flex justify-center items-center">
      ...
      <form
        className="h-[80px] px-4 flex items-center bg-white border-t border-gray-200"
        onSubmit={sendMessage}
      >
        <input
          className="w-full bg-white text-gray-900 border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-300 placeholder:text-gray-400"
          value={msg}
